...
def CreatModel(args, num_features, n_cls, data_x,device, num_edges=None):
    ...
    # elif args.net.lower() == 'linkxgit':
        ...
        if args.net == 'GCN':
            ...
            model = create_gat(nfeat=num_features, nhid=args.hid_dim, nclass=n_cls, dropout=args.dropout, nlayer=args.layer, head=args.heads)
...
def get_name(args, IsDirectedGraph):
    ...
    if args.all1 and not args.degfea:
        dataset_to_print += 'all1'
    elif args.degfea:
        dataset_to_print += 'degfea' + str(args.degfea)
    ...
    if args.net == 'LargeScaleNet':
        net_to_print += ('_' + args.conv_type2 + '_part' + str(args.alphaDir) + '_' + str(args.betaDir) + '_' + str(
            ...
            + '_zero'+str(args.zero_order) + '_struct'+str(args.structure)) + '_cat'+str(args.cat_A_X)+'_exp'+str(args.exponent)+args.att+str(args.att_head)

    if args.net in ['ScaleNet', 'Dir-GNN']:
        if args.coef_agg:
            net_to_print += 'coef_agg' + str(args.coef_agg)
